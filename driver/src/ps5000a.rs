...
use pico_common::{
    ...
    PicoInfo, PicoRange, PicoStatus, PicoVerticalResolution, TimeUnits, ToPicoStr,
};
use pico_config::{
...
impl PS5000ADriver {
    ...
    pub fn maximum_adc_value(&self, handle: i16) -> PicoDriverResult<i16> {
        ...
        PicoStatus::from(unsafe { self.bindings.ps5000aMaximumValue(handle, &mut value) })
            .to_result(value, "maximum_adc_value")
    }
    ...
    pub fn set_vertical_resolution(
        ...
        PicoStatus::from(unsafe {
            self.bindings
                .ps5000aSetDeviceResolution(handle, resolution.into())
        })
