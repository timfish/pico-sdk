...
impl Driver {
    ...
    pub fn from_pid(pid: u16) -> Option<Driver> {
        match pid {
            ...
            0x100E | 0x1204 | 0x1215 | 0x1216 | 0x12A0 | 0x12A1 => Some(Driver::PS6000),
            u => {
                warn!("Unsupported Pico Product ID found: {:#X}", u);
                None
            }
        }
    }
    ...
    /// Gets the symbol name for this driver
    /// ```
    /// use pico_common::Driver;
    ///
    /// let driver = Driver::PS5000A;
    /// let sym_name = driver.get_symbol_name(b"OpenUnit");
    ///
    /// assert_eq!(sym_name, b"ps5000aOpenUnit");
    ///
    /// let driver = Driver::PS2000;
    /// let sym_name = driver.get_symbol_name(b"_open_unit");
    ///
    /// assert_eq!(sym_name, b"ps2000_open_unit");
    /// ```
    pub fn get_symbol_name(self, sym: &[u8]) -> Vec<u8> {
        [self.to_string().as_bytes(), sym].concat()
    }
}
