...
/// Error codes returned by `usb_tc08_get_last_error`
///
/// The TC-08 driver has its own error space rather than using `PicoStatus`, so these are mapped
/// onto the nearest `PicoStatus` by [`TC08Error::to_status`] to keep one error type across
/// every instrument family.
#[allow(non_camel_case_types)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
